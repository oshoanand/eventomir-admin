...
// Expanded to include your specific performer icons
...
export default function SettingsPage() {
  ...
  // ADDED 'link' to the form data state
  const [categoryFormData, setCategoryFormData] = useState<{
    name: string;
    icon: string;
    link: string;
  ...
  const openAddCategoryDialog = () => {
    setEditingCategory(null);
    setCategoryFormData({ name: "", icon: "Star", link: "" });
    setIsCategoryDialogOpen(true);
  };

  const openEditCategoryDialog = (category: SiteCategory) => {
    setEditingCategory(category);
    // Include link when editing
    setCategoryFormData({
      name: category.name,
      icon: category.icon,
      link: (category as any).link || "",
    });
    setIsCategoryDialogOpen(true);
  };

  const handleSaveCategory = () => {
    // Validate both name and link are present
    if (
      !settings ||
  ...
  // ... (Pricing & SEO handlers remain the same)
  const handlePriceChange = (
    planId: string,
    duration: "monthly" | "halfYearly" | "yearly",
    value: string,
      ...
      return {
        ...
        plans: prev.plans.map((plan) =>
          plan.id === planId
            ? {
                ...plan,
                features: [...(plan.features || []), "Новая возможность"],
              }
            : plan,
        ),
      };
  ...
  return (
    <div className="container mx-auto pt-2 pb-10 max-w-6xl">
      ...
      <Tabs defaultValue="general" className="space-y-6">
        ...
        {/* ... (GENERAL, DESIGN, CONTACTS TABS REMAIN UNCHANGED - REMOVED FOR BREVITY IN SNIPPET, BUT KEEP IN YOUR CODE) ... */}
          ...
          <Card className="shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Услуги, отображаемые на главной странице сайта с привязкой к
                  фильтрам поиска.
                </CardDescription>
              </div>
              <Button
            ...
            <CardContent>
              <div className="border rounded-lg bg-slate-50/50 p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {settings.siteCategories?.length > 0 ? (
                  settings.siteCategories.map((category) => (
                    <div
                      key={category.id}
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className="shrink-0 p-2 bg-primary/5 rounded-md">
                          {renderIcon(category.icon, {
                            className: "h-5 w-5 text-primary",
                          })}
                        </div>
                        <div className="flex flex-col truncate pr-2">
                          ...
                          {/* Display the link below the category name */}
      ...
      {/* Category Dialog Updated with Link */}
      <Dialog
        open={isCategoryDialogOpen}
        onOpenChange={setIsCategoryDialogOpen}
      >
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Название (Отображается на сайте)</Label>
              <Input
                value={categoryFormData.name}
                onChange={(e) =>
                  setCategoryFormData((p) => ({ ...p, name: e.target.value }))
            ...
            <div className="space-y-2">
              <Label>Ссылка для поиска (Link)</Label>
              <Input
                value={categoryFormData.link}
                onChange={(e) =>
                  setCategoryFormData((p) => ({ ...p, link: e.target.value }))
                }
                placeholder="/search?category=Фотограф"
              />
              <p className="text-[11px] text-muted-foreground leading-tight">
                URL, по которому перейдет пользователь. Используйте параметры
                фильтрации.
              </p>
            </div>
