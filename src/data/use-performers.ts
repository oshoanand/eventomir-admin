...
const getPerformers = async (
  ...
  const response = await apiRequest({
    method: "get",
    url: `/api/users/performers${queryString}`,
  });
  return response as PerformersResponse;
};

export function usePerformersQuery(page: number, limit: number) {
  return useQuery({
    queryKey: ["performers", page, limit],
    queryFn: () => getPerformers(page, limit),
