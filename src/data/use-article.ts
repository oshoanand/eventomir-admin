...
const fetchAdminArticlesFn = async (): Promise<Article[]> => {
  return await apiRequest<Article[]>({
    method: "get",
...
const fetchAdminArticleByIdFn = async (id: string): Promise<Article> => {
  return await apiRequest<Article>({
    method: "get",
    url: `/api/articles/admin/${id}`,
  });
};
...
export const useAdminArticle = (id: string) => {
  return useQuery({
    ...
    queryFn: () => fetchAdminArticleByIdFn(id),
    enabled: !!id,
  });
};
