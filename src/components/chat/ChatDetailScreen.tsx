...
export default function ChatDetailScreen({
  ...
  const allMessages = useMemo(() => {
    ...
    // Safely extract messages whether the page is an array or an object like { messages: [] }
    ...
    // Sort chronologically (oldest first) so they render top-to-bottom
  ...
  const injectOptimisticMessage = (msg: ChatMessage) => {
    const queryKeyVariants = [
      ...
      ["chatHistory", userId, partnerId], // Tries both standard key formats
    ];
    ...
    queryKeyVariants.forEach((key) => {
      queryClient.setQueryData(key, (oldData: any) => {
        ...
        const newPages = oldData.pages.map((page: any) => {
          ...
          return page.filter((m: ChatMessage) => m.id !== messageId);
        });
        return { ...oldData, pages: newPages };
      });
    });
